...
The complete term sets, MeSH descriptors, and inclusion logic are in `config/query_terms.yml` in the project repository, and the resolved journal list is in `config/journals_core.yml`.
...
SRED contains {{corpus.n_records|,}} unique article records with abstracts, published between {{corpus.year_min}} and {{corpus.year_max}} across {{corpus.n_journals|,}} journals. Records carrying a {{corpus.year_max}} publication date are ahead-of-print items deposited before the harvest and are excluded from all trend analyses. DOIs are present for {{corpus.pct_with_doi}}% of records and citation counts for {{corpus.pct_with_citations}}%.
